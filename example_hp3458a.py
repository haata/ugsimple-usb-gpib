...
import sys
import time
from ugsimple.GPIB import UGSimpleGPIB as ugs_gpib
import binascii
...
class hp3458a(object):

	def __init__(self, name = "gpib0", adr = 22):
                self.gpib = ugs_gpib(debug_mode=DEBUG_MODE, timeout=30000)
                self.gpib.query_devices()
                self.gpib_addr = adr
        ...
        def wr(self, data):
                self.gpib.write(self.gpib_addr, data)
                time.sleep(0.1)

        def ask(self, data):
            return self.gpib.ask(self.gpib_addr, data, method='retry')

        def wait_cmd(self, some):
                pass
